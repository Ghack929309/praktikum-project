...
const deleteCard = (req, res) => {
  ...
  Card.findById(id)
    ...
    /*
    *  1- All requests don't get the correct response because
    *     here we are trying to update when we are intended to
    *     to delete.
    * */
    .then((card) => Card.deleteOne(card))
      .then((res) => {
        if (res.deletedCount >= 1) res.status(200).send({message: 'Card' +
            ' deleted'})
    })
